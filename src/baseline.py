...
from forecasting_models import SNAIVE, SMM

import argparse

from cfg import TEST_SIZE, VAL_SIZE, DAILY_WINDOW, DAILY_CONTEXT, DAILY_SEASONALITY, HOURLY_WINDOW, HOURLY_CONTEXT, HOURLY_SEASONALITY, PRED_PATH, TS_PATH, DATA_PATH
...
if __name__ == "__main__":
    ...
    for (application, metric, aggregation, type, use), sub_df in df.groupby(["application", "metric", "aggregation", "type", "use"]):
        ...
        models = create_models(window = window, context = context, seasonality = seasonality)
    ...
    pd.DataFrame(exp_metadata).to_csv(save_in / f"baseline_{args.aggregation}.csv", index=None)